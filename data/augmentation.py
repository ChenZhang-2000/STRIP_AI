...
transform = iaa.Sequential([rotate, noise, crop])
...
if __name__ == "__main__":
    ...
    to_directory = rf'E:\Datasets\STRIP_AI\processed\Avg_Aug_7'
    sample_size = 2500
    ...
    draw_ce = np.random.choice(ce_df['patient_id'], sample_size)
    draw_laa = np.random.choice(laa_df['patient_id'], sample_size)

    times_count = defaultdict(lambda: 1)
    for n, samples in enumerate((draw_ce, draw_laa)):
        for idx, patient_id in enumerate(samples):
            Path(f"{to_directory}\\{patient_id}_{times_count[patient_id]}").mkdir(parents=True, exist_ok=True)
            img_data = []
            file_names = []
            last_img_num = -1
            for file_name in os.listdir(directory + f"\\{patient_id}"):
                ...
                gd = get_gradient(torch.from_numpy(aug_image).transpose(1, 2).transpose(0, 1)).detach()
            ...
            torch.save((out_tensor, index_tensor), f"{to_directory}\\{patient_id}_{times_count[patient_id]}\\gradient.gd")
            times_count[patient_id] += 1

            if (idx+1) % (sample_size//5) == 0:
                print(f"Iteration: {idx+1+n*sample_size}/{sample_size*2}")
